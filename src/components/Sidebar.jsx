...
export default function Sidebar({ workspaces, activeWorkspace, setActiveWorkspace, activeTool, setActiveTool, user, logout, onAddWorkspace }) {
  ...
  return (
    <aside className="w-full h-full bg-white/80 dark:bg-black/80 backdrop-blur-md border-r border-slate-200 dark:border-white/5 p-6 flex flex-col gap-8">
      <div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0b0d] border border-white/10 p-6 rounded-2xl w-full max-w-sm relative">
            <button type="button" onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-white/40 hover:text-white">
              <X size={20} />
            </button>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 focus:outline-none focus:border-blue-500/50 text-sm"
                />
              </div>
              <div>
