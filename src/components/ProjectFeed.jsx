...
export default function ProjectFeed() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-violet-900/20 via-primary/5 to-cyan-500/10 border border-white/5 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden backdrop-blur-xl">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-11 pr-4 focus:outline-none focus:border-primary text-sm text-right rtl-text text-white placeholder:text-slate-500"
          />
        </div>
        ...
        {(searchTerm || selectedTag) && (
          <button
            ...
            className="px-4 py-3 bg-white/5 border border-white/10 rounded-2xl text-xs text-slate-400 hover:text-white transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
          >
            إلغاء التصفية
          </button>
        )}
      ...
      {allTags.length > 0 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          <span className="text-xs text-slate-500 font-semibold whitespace-nowrap">القطع الأكثر استخداماً:</span>
          <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((idx) => (
            <div key={idx} className="bg-white/[0.01] border border-white/5 rounded-3xl p-6 h-64 animate-pulse flex flex-col justify-between">
        ...
        <div className="text-center py-16 bg-white/[0.01] border border-white/5 rounded-3xl p-8 backdrop-blur-xl">
          <FolderCode className="w-16 h-16 text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-white">لم يتم العثور على أي مشاريع</h3>
          <p className="text-xs text-slate-500 mt-2">جرب البحث بكلمات أخرى أو إلغاء مرشحات البحث النشطة.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((proj) => (
            <div
              ...
              className="group bg-white/[0.02] hover:bg-white/[0.04] border border-white/10 hover:border-primary/50 rounded-3xl p-6 transition-all duration-300 flex flex-col justify-between h-[340px] relative overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.4)] hover:shadow-primary/5 cursor-pointer"
            >
              ...
              <div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-1.5 text-[10px] text-slate-500 dark:text-zinc-500 font-mono">
                    <Calendar size={11} />
                    <span>{new Date(proj.createdAt).toLocaleDateString()}</span>
                  </div>
                  {proj.images && proj.images.length > 0 ? (
                    <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/10 shadow-sm shrink-0">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-end mt-4 line-clamp-1 h-6">
                  {proj.componentsList && proj.componentsList.slice(0, 3).map((tag, idx) => (
                    <span
                      key={idx}
                      className="text-[9px] font-bold px-2 py-1 rounded-lg bg-zinc-900 border border-white/5 text-slate-400 group-hover:border-primary/20 group-hover:text-primary transition-colors whitespace-nowrap"
                    >
                      {tag}
                    </span>
                  ))}
                  {proj.componentsList && proj.componentsList.length > 3 && (
                    <span className="text-[9px] font-bold px-1.5 py-1 text-slate-500 whitespace-nowrap">
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-4">
                  ...
                  {/* Metrics list */}
                  <div className="flex items-center gap-3 text-[10px] text-slate-500 dark:text-zinc-500 font-mono">
                  ...
                  <div className="flex items-center gap-2 cursor-pointer" onClick={(e) => {
                    ...
                    <span className="text-xs text-slate-400 hover:text-white font-medium transition-colors">
                      @{proj.ownerUsername}
                    </span>
                    <div className="w-5 h-5 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center text-[9px] font-bold text-white uppercase shadow-sm">
